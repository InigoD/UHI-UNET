...
folderModels = '/home/jdelser/Work/UNET-NEW/MODELS/LWT_ALL_HOURS/'
...
def create_dataset_for_some_dates(dataset, lookback, selectedDates):
    if(len(np.shape(dataset['data']))>2):
        X = [np.transpose(dataset['data'][i-lookback:i+1],[1,2,0,3]).reshape(IMG_SIZE_X,IMG_SIZE_Y,-1) for i in range(lookback, len(dataset['data'])) 
            if dataset['datetime'][i].strftime("%Y-%m-%d") in selectedDates]
    else:
        X = [dataset['data'][i-lookback:i+1] for i in range(lookback, len(dataset['data'])) 
            if dataset['datetime'][i].strftime("%Y-%m-%d") in selectedDates]
    return X

def create_target_variable_for_some_dates(dataset, lookback, selectedDates):
...
for ipatch in tqdm(PATCHES_TRAIN):
    for year in YEARS_TRAIN:
        Xspatial_train.extend(create_dataset_for_some_dates(dataSpatialDict[ipatch][year],LOOKBACK,listLWT))
        Y_train.extend(create_target_variable_for_some_dates(dataTaDict[ipatch][year],LOOKBACK,listLWT))
        Xmeteo_tmp = []
        for iClimate in range(len(namesClimate)):
            Xmeteo_tmp.append(create_dataset_for_some_dates(dataClimateDict[namesClimate[iClimate]][year],LOOKBACK,listLWT))
...
#         Xspatial_val.extend(create_dataset_for_some_dates(dataSpatialDict[ipatch][year],LOOKBACK,listLWT))
#         Y_val.extend(create_target_variable_for_some_dates(dataTaDict[ipatch][year],LOOKBACK,listLWT))
#         Xmeteo_tmp = []
#         for iClimate in range(len(namesClimate)):
#             Xmeteo_tmp.append(create_dataset_for_some_dates(dataClimateDict[namesClimate[iClimate]][year],LOOKBACK,listLWT))
...
#         Xspatial_test.extend(create_dataset_for_some_dates(dataSpatialDict[ipatch][year],LOOKBACK,listLWT))
#         Y_test.extend(create_target_variable_for_some_dates(dataTaDict[ipatch][year],LOOKBACK,listLWT))
#         Xmeteo_tmp = []
#         for iClimate in range(len(namesClimate)):
#             Xmeteo_tmp.append(create_dataset_for_some_dates(dataClimateDict[namesClimate[iClimate]][year],LOOKBACK,listLWT))
...
unet_model.save(folderModels + 'unet_trained_over_LWT_of_' + str(YEARS_TRAIN) + '.keras')

with open(folderModels + 'unet_trained_over_LWT_of_' + str(YEARS_TRAIN) + '_history.pkl', 'wb') as file_history:
