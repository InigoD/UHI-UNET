...
SELECTEDWINDOW = 57 # 27,12,32,34,57,29
ACRONYM = 'PUNTAGALEA'
patch = SELECTEDWINDOW

for PERCENTIL in ['01','03','05','07','09','095']:
    ...
    VMIN = -8 # Valor minimo para la escala de colores
    VMAX = 8
    ...
    for SELECTEDHOUR in range(24):
        ...
        dataToPlot_UNET = testData[patch]['UHI_INDEX_PER_HOUR_P_'+PERCENTIL][SELECTEDHOUR].ravel()

        dataAll.extend(dataToPlot_UNET)
        ...
        axs[irow,icol].set_xlabel('UHI, '+str(SELECTEDHOUR)+':00',fontsize=16)
    ...
    plt.savefig(FOLDER_PLOTS+PERCENTIL+'PERCENTILE_UHI_over_LWT_days_over_'+ACRONYM+'_patch_'+str(patch)+'.pdf')
    plt.close()
